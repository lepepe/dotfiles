...
with open(pykolors) as f:
    kolors = json.loads(f.read())
...
keys = [
    ### The essentials
    Key([mod], "Return", lazy.spawn(myTerm), desc=f"Launches: {myTerm}"),
    Key([mod], "d", lazy.spawn("rofi -show run"), desc="Rofi Application Launcher"),
    ...
    Key([mod, "shift"], "b", lazy.spawn("qutebrowser"), desc="Launches Qutebrowser"),
    Key(
        [mod, "mod1"], "e", lazy.spawn(myTerm + " -e neomutt"), desc="Launches neomutt"
    ),
    Key(
        [mod, "shift"],
        "s",
    ...
    Key(
        ["mod1", "control"], "f", lazy.spawn("flameshot gui"), desc="Launches flameshot"
    ),
    Key(
        [mod, "control"],
        "k",
        lazy.spawn(
            myTerm + " -e python " + os.path.expanduser("~/.config/qtile/keys.py")
        ),
...
widget_defaults = dict(
    font="DejaVu Sans Mono", fontsize=12, padding=2, background=kolor02
)
...
def init_widgets_list():
    widgets_list = [
        ...
        widget.TextBox(
            ...
            background=kolorbg,
            fontsize=20,
            mouse_callbacks={"Button1": lambda: qtile.spawn("rofi -show run")},
        ),
        widget.GroupBox(
        ...
        widget.Sep(linewidth=0, padding=20, foreground=kolor02, background=kolor00),
        widget.WindowName(foreground=kolor05, background=kolor00, padding=0),
        widget.TextBox(
            text="",
            padding=5,
            foreground=kolor05,
        ...
        widget.Maildir(
            ...
            mouse_callbacks={"Button1": lambda: qtile.spawn(myTerm + " -e neomutt")},
            update_interval=60,
            padding=5,
            foreground=kolor05,
            background=kolorbg,
        ...
        widget.ThermalSensor(
            foreground=kolorfg, background=kolor08, threshold=90, padding=5
        ),
        widget.TextBox(
            text="| ",
            foreground=kolorfg,
        ...
        widget.GenPollText(
            ...
            foreground=kolorfg,
            mouse_callbacks={
                "Button1": lambda: qtile.spawn(myTerm + " -e sudo pacman -Syu")
            },
            background=kolor13,
        ),
        widget.TextBox(
        ...
        widget.Wlan(
            ...
            padding=5,
            mouse_callbacks={
                "Button1": lambda: qtile.spawn(myTerm + " -e nmcli device wifi list")
            },
        ),
        widget.TextBox(text="Vol:", foreground=kolorfg, background=kolor13, padding=5),
        widget.Volume(foreground=kolorfg, background=kolor13, padding=5),
        widget.Battery(
            foreground=kolorfg,
            background=kolor08,
            padding=5,
        ...
        widget.CurrentLayout(foreground=kolorfg, background=kolor13, padding=5),
        widget.Clock(
            foreground=kolor01,
            background=kolorbg,
            format=" %m/%d -  %H:%M",
        ...
        widget.Systray(background=kolor00, padding=5),
    ]
    return widgets_list
...
def bottom_widgets_list():
    widgets_list = [
        ...
        widget.GenPollUrl(
            ...
            parse=lambda data: "$" + str(round(data["bpi"]["USD"]["rate_float"])),
            update_interval=600,
            foreground=kolor05,
            background=kolorbg,
            padding=5,
        ...
        widget.TextBox(
            text="|", padding=5, foreground=kolor05, background=kolorbg, fontsize=12
        ),
        widget.GenPollText(
            background=kolorbg,
            foreground=kolor05,
            ...
            padding=5,
            func=lambda: subprocess.check_output(
                os.path.expanduser("~/.local/bin/scripts/stock.py")
            ).decode(),
            fontsize=12,
        ),
        widget.Sep(linewidth=0, padding=6, foreground=kolorfg, background=kolorbg),
